      ...
      <div className="flex gap-6">
        <div className="flex-2 bg-white rounded-xl shadow-sm border border-gray-200">
          <div className="flex items-center justify-between p-5 border-b border-gray-200">
            ...
            <button 
              onClick={() => onNavigate('/provider/jobs')}
              className="text-brandText text-sm hover:underline font-medium"
            >
              View All →
            </button>
          </div>
          <div className="p-4 space-y-3">
            {recentJobs.map((job) => (
              <div 
                key={job.id} 
                className="p-4 hover:bg-gray-50 transition-colors border border-gray-200 rounded-xl"
              >
          ...
          <div className="p-5">
            ...
            <div className="space-y-3">
              {actionItems.map((item) => (
                <div 
                  key={item.id}
                  className={`${item.bgColor} ${item.hoverBg} rounded-xl border ${item.urgent ? 'border-red-200' : 'border-gray-200'} py-8 px-5 transition-all duration-200 cursor-pointer hover:shadow-md`}
                >
                  <div className="flex items-center gap-3">
                    ...
                    <div className="flex-1">
                      <div className="flex items-center justify-between">
                        <span className={`text-sm ${item.urgent ? 'text-red-700' : 'text-gray-700'}`}>
                          {item.title}
                        </span>
                        {item.urgent && (
                ...
                {goal.rating && (
                  <>
                    ...
                    <div className="w-full bg-gray-200 rounded-full h-1.5">
                      <div 
                        className="bg-brandText rounded-full h-1.5 transition-all duration-300"
                      ></div>
                    </div>
        ...
        <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
          ...
          <div className="space-y-3">
            {clientInsights.map((insight, index) => (
              <div 
                key={index} 
                className="bg-gray-100 rounded-xl p-4 hover:bg-gray-100 transition-colors cursor-pointer"
              >
                <div className="flex items-center justify-between">
                  <span className="text-gray-700 text-sm font-medium">{insight.label}</span>
                  <span className="text-gray-900 text-lg font-bold">{insight.value}</span>
                </div>
              </div>
            ))}
          </div>
